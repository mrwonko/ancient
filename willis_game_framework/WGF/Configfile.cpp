...
namespace WGF
{
    ...
    bool Configfile::SaveToFile(const std::string& filename, bool clear)
    {
        if(clear) //clear file prior to writing
        {
        ...
        //else - overwrite existing values, add non-existing to the end
        std::ifstream src(filename.c_str());
        if(file.fail()) //if the file could not be opened, there is no such file. Therefore we create a new file so we can just as well use the clear method.
        {
            return SaveToFile(filename, true);
        }
        std::string content; //content of the file, will be altered to get the changes
        //read the file
        while(!src.eof())
        {
            content += src.get();
        }
        //TODO SaveToFile with clear = false
